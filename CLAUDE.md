...
| Path | Purpose |
|---|---|
| `macro/schema.sql` | Tables, views. Plain tables — see traps. Must match the live table (trap 54) |
...
54. **A column with no consumer is a trap, and a half-populated one is a worse
    one.** `pub_lag_days` and `staleness_mode` sat in `macro_series_meta` from
    the retired 0018 seed and were **read by nothing** -- the only mentions
    anywhere were the schema defining them, the seed inserting them, and the
    column list copying them across. `staleness_mode` was NULL on all 3,126
    rows. `pub_lag_days` was set on 18, every one to the same **80**, which is a
    placeholder rather than a measurement -- and that is the worse case, because
    it invites a reader to believe 18 series have a known publication lag while
    3,108 do not, when in truth none do. Both dropped 6 September 2026.

    The check they were meant to serve already exists and works from each
    series' own frequency with a grace period **imported** from `coverage.py`
    rather than restated, which is the pattern to keep. If a per-series
    publication lag is ever genuinely wanted, **measure it from the vintages
    already held** -- the gap between an observation's date and the vintage that
    first carried it -- rather than typing it into a column where nothing will
    notice it rotting.

    Two practical notes. `macro/catalog.sql` is **generated** and says so at the
    top, so it is regenerated by `gen_catalog.py`, never hand-edited; and
    `gen_catalog.py`'s `COLS` still lists both names because that describes
    0018's column order, which is fixed history and must keep matching the file
    being parsed. The pipeline connects as `macro_app`, which deliberately has
...
- **Reading text is 10% larger** (14 September 2026): notes,
  dash points and the forecast write-up 15px, the lede 19px. The gist
  under each heading went to 17px too, then down to 14.5px the same day at
  William's request. Headings,
